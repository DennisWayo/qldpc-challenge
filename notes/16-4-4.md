# [[16,4,4]] — SAT-discovered weight-6 single-layer checkerboard code (a)
...
whose check matrix matches `codes/16-4-4.json` (fingerprint
3440fc480e2f); the full sweep parameters are in
...
- Exact certification in staging: `verify/certify.py` (scipy/HiGHS MILP, one
  ...
  codes/16-4-4.json`; the board tier upgrades only on that server-side run.
...
`codes/16-4-4.json` are the construction. To regenerate the family: enumerate
...
`uv run python verify/qldpc_verify.py codes/16-4-4.json`.
