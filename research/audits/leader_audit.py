#!/usr/bin/env python3
"""Deep fresh-seed RIS audit of board entries: is a leader's distance real, or soft?
...
def ris(HX, HZ, trials, seed, threads=8, pair_depth=10):
    ...
    if surrogate._fast is not None:
        w, side, support = surrogate._fast.distance_rand_witness(
            ...
            trials=int(trials), seed=int(seed), pair_depth=pair_depth,
            threads=int(threads))
    ...
    wx, sx = surrogate._search_lightest(*prepared.side("X")[:2], trials, seed,
                                         bases=prepared.side("X")[2:])
    wz, sz = surrogate._search_lightest(*prepared.side("Z")[:2], trials, seed + 1,
                                         bases=prepared.side("Z")[2:])
...
def describe(n, k_claim, HX, HZ, doc, tag):
    ...
    print(f"{tag}: n={n} k={k} (claimed {k_claim}) w={w} css_ok={verify_css(HX, HZ)}"
...
def cmd_ladder(args):
    ...
    best = {"weight": n + 1, "side": None, "support": [], "seed": None,
            "trials": None}
    for trials, seeds in args.ladder:
        for seed in seeds:
            ...
            print(f"  trials={trials:>10,} seed={seed}: d<={weight} side={side} "
                  f"[{dt:.0f}s] witness={why}", flush=True)
            if weight < best["weight"]:
                best = {"weight": weight, "side": side, "support": support,
                        "seed": seed, "trials": trials}
                print(f"    NEW BEST d<={weight} side={side}", flush=True)
    claim = doc["distance"]["d"]
    verdict = ("REFUTED" if best["weight"] < claim
               else "holds" if best["weight"] == claim else "inconclusive")
    print(f"VERDICT: n={n} k={k} claim={claim} d_ub={best['weight']} "
...
def cmd_screen(args):
    ...
    for entry in args.entries:
        ...
        for seed in args.seeds:
            ...
            ok, why = validate_witness(n, HX, HZ, side, weight, support)
            print(f"  seed={seed}: d<={weight} side={side} [{dt:.0f}s] "
                  f"witness={why}", flush=True)
            best = weight if best is None else min(best, weight)
        verdict = ("REFUTED" if best < claim
                   else "holds" if best == claim else "inconclusive")
        rows.append((os.path.basename(entry)[:-5], n, k, w, claim, best, verdict))
    print("=" * 72)
    print(f"{'entry':>14s} | {'n':>4s} {'k':>4s} {'w':>2s} {'claim':>5s} "
          f"{'d_ub':>5s}  verdict")
...
def main(argv=None):
    ...
    lad.add_argument("--ladder", nargs="+", required=True,
                     metavar="TRIALS:SEED,SEED", type=str,
